...
export default async function HomePage() {
  ...
  const [residenceRes, bookingsRes] = await Promise.all([
    supabase
      .from("residences")
      .select(
        "id, name, description, address, facebook_page_id, latitude, longitude, residence_amenities(id, name), residence_inclusions(id, description)",
      )
      .limit(1)
      .maybeSingle(),
    supabase
  ...
  const residence = residenceRes.data as ResidenceFull | null;
  const bookings = (bookingsRes.data ?? []) as BookingWithUnit[];

  /* Fetch units with images only if we have a residence */
  ...
  if (residence?.id) {
    const unitsRes = await supabase
      ...
      .order("name");
    if (unitsRes.data) {
      unitsWithImages = (unitsRes.data as UnitWithImages[]).map((u) => ({
        ...u,
    ...
    <div className="bg-white min-h-screen">
      ...
      >
        <div className="absolute inset-0">
          <Image src="/images/Cover.png" alt="Bitara Residence" fill className="object-cover" priority />
        </div>
        ...
        <div className="relative z-10 px-6 max-w-2xl mx-auto space-y-5">
          ...
          >
            {residence?.name ?? "Bitara's Residence"}
          </h1>
      ...
      <section id="amenities" className="py-10 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center text-2xl uppercase tracking-wider font-bold">Amenities</h2>
          <hr className="mt-3" />
      ...
      <section id="inclusions" className="py-10 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center text-2xl uppercase tracking-wider font-bold">What&apos;s Included</h2>
          <hr className="mt-3" />
          <div className="mt-8 bg-gray-50 rounded-3xl p-7">
            {inclusions.length > 0 ? (
      ...
      <section id="gallery" className="py-10 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center text-2xl uppercase tracking-wider font-bold">Unit Tour</h2>
          <hr className="my-3" />
          <SectionLabel>Unit Gallery</SectionLabel>
          <div className="mt-8">
      ...
      <section id="bookings" className="py-10 px-6 bg-gray-100">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center text-2xl uppercase tracking-wider font-bold">Booking Calendar</h2>
          <hr className="my-3" />
      ...
      <section id="location" className="py-10 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center text-2xl uppercase tracking-wider font-bold">Where to Find Us</h2>
          <hr className="my-3" />
      ...
      <footer id="contact" className="bg-[#1e3a5f] text-white py-10 px-6">
        <div className="max-w-3xl mx-auto text-center space-y-10">
          ...
          <div className="border-t border-white/10 pt-8 text-white/30 text-xs">
            © {new Date().getFullYear()} Bitara Residence. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
  ...
  return (
    <span
      className={`inline-block px-4 py-1.5 rounded-full text-sm font-semibold ${
        light ? "bg-white text-gray-700 shadow-sm" : "bg-gray-100 text-gray-700"
      }`}
    >
      {children}
    </span>
